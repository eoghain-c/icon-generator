...
function tinymce_icon_generator_script( $plugin_array ) {
	$plugin_array['icon_button_script'] = get_stylesheet_directory_uri() . 'icon-generator.js.php';  // TODO Change this to reflect the path/filename to your js file
	return $plugin_array;
}
...
function generate_icons($atts, $content){
    ...
    if (!empty($atts['icon'])) {
        if(file_exists($_SERVER['DOCUMENT_ROOT'] . ' // TODO update path '. $atts['icon'] .'.svg')){ ?> 
           ...
           <?php return '
            <div class="generated-icon '. $styleClass .'">
                <div class="icon-container">
                    ...
                    <img class="generated-icon__svg ' . $atts['colour'] .' lazyload" src="'. get_stylesheet_directory_uri() .'// TODO add path' . $atts['icon'] . '.svg" alt="' . $atts['alt'] . '">
